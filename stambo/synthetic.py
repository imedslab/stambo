...
def generate_n_measurements(n_subjects: int, n_measurements: int, random: bool=True, d_temp: float=3, min_samples: int=1, seed: int=None) -> npt.NDArray[np.int64]:
    """Generates the random number of measurements per subject.
    
    Args:
        n_subjects (int): Total number of subjects.
        n_measurements (int): Total number of measurements.
        random (bool, optional): Whether to generate random number of measurements per subject. Defaults to True.
        d_temp (float, optional): Parameter for the Dirichlet distribution. Defaults to 3.
        min_samples (int, optional): Minimum number of measurements per subject. Defaults to 1.
        seed (int, optional): Seed for the random number generator. Defaults to None.
    Returns:
        np.ndarray: An array of shape (M,) containing the number of measurements generated per subject.
    """
    if seed is not None:
        np.random.seed(seed)

    # Ensure that every subject gets at least one measurement
    subject_measurements = np.ones(n_subjects, dtype=int) * min_samples  # Start with min_samples measurements per subject
    remaining_measurements = n_measurements - n_subjects * min_samples  # Distribute the remaining measurements
    if random:
...
def generate_subject_measurements(subject_measurements: npt.NDArray[np.int64], mean_range: tuple, base_cov: npt.NDArray[np.float64], seed: int=None):
    """
    Simulates a dataset from `subject_measurements` subjects with a total of `n_measurements` measurements.
    ...
    Args:
        subject_measurements (np.ndarray): Array of shape (M,) containing the number of measurements per subject.
    ...
    data = []
    subject_ids = []
    subject_means = [np.random.uniform(mean_range[0], mean_range[1], size=dim) for _ in range(subject_measurements.shape[0])]  # Unique mean per subject
    ...
    for subject_id, num_measurements in enumerate(subject_measurements):
        ...
        subject_data = np.random.multivariate_normal(subject_means[subject_id], subject_cov, size=num_measurements)
...
def generate_paired_data(n_subjects: int, n_measurements: int, effect_size: float=0.5, dim: int=1, min_samples: int=2, d_temp=2, mean_range: tuple=None, base_cov: np.ndarray=None, seed: int=None):
    ...
    data, subject_ids, _ = generate_subject_measurements(n_measurements_per_subj, mean_range, base_cov)
