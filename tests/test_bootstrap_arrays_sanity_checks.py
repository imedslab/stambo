...
from stambo._stambo import bootstrap_arrays
...
def test_bootstrap_arrays_accepts_valid_numpy_inputs():
    ...
    np.random.seed(0)
    out = bootstrap_arrays(arrays=arrays, statistics={"mean": np.mean}, n_bootstrap=3, silent=True)
    assert out["mean"].shape == (3, 2)


def test_bootstrap_arrays_accepts_valid_predsamples_inputs():
    gt = np.array([0, 1, 0, 1], dtype=int)
    ...
    np.random.seed(0)
    out = bootstrap_arrays(arrays=(w1, w2), statistics={"mean": lambda s: float(np.mean(s.gt))}, n_bootstrap=3, silent=True)
    assert out["mean"].shape == (3, 2)
